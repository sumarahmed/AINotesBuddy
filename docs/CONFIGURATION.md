...
`2026.08.8`; `package.json` represents it as `2026.8.8` for semantic-version
compatibility.
...
| Product version | `2026.08.8` | Website/package `Year.Month.MinorRelease` in `src/runtime-config.js` and `package.json` |
| Latest companion version | `2026.08.4` | Public, non-secret value in `src/runtime-config.js` used for existing-user update warnings |
